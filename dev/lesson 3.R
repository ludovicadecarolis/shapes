...
###call as the .R file!
###we are creating the test folder: we have the testhat file that is an initialization for the tests and inside testhat folder: test-division file
###press CTRL + SHIFT + T (TEST)
###there is one test
###test_that("my_division works", {expect_equal(4/2, 2)}) cioe mi aspetto che 4/2 dia 2
###test_that("my_division works", {expect_equal(4/2, 3)}) ----> viene fuori FAILURE: actual e expected vengono forniti
###sensato: test_that("my_division works", {expect_equal(my_division (4,2), 2)})
###->fix the mistake
###u can create more test in the same context
###test_that("my_division works with real numbers", {expect_equal(my_division (10,3), 3.33333333)})
###u can write also--->my_division (10,3), 3.3,tolerance=0.1)})---> without tolerance is wrong! because there is just 1 decimal
###is useful because when u change the code then u run all the tests and u are sure all is going correctly

## Add internal datasets (not required for u!)
## If you want to provide data along with your package
usethis::use_data_raw( name = "my_dataset", open = FALSE )
###create a my_dataset.R inside data-raw folder--> is the procedure to create ur data: usethis::use_data(my_dataset, overwrite = TRUE)
###we can create a dataset: taking mtcars bringing row names to 1 column and transform it as a tibble---> put in: my_dataset.R:
###library(tibble) ---and--- my_dataset<-as_tibble(rownames_to_column(mtcars,var='car'))
###I prove it the console
###I source this script (in alto a dx) and it creates a new folder data with inside my_dataset.rda
###clean the environment
###build and load the package
###so u can see that the datset is not in the environment but is provided by the package!!
###u cannot provide big dataset but u can have example dataset that can be use to create a vignette!
...
### ```
###--> i am providing a fc with a example dataset

devtools::build_vignettes()
###build a vignette
###-->creates a doc folder with html, R, Rmd files
###install and reload the package

# Install your package manually:
devtools::install(build_vignettes = TRUE)
###I can access to my vignette
vignette(package = "shapes") ###--> viene fuori messaggio con le vignettes del pacchetto
###and call
vignette("ThisTidyPackage") ###-->example of use of ur package in the help
###for example:
vignette(package = "tidyr")
vignette('pivot',package ="tidyr") ###-->viene fuori nell help la vignette della pivot
###2 ways of deliver documentation: roxygen (document each fc) and creating a vignette (create an example of usage)

###go to the GITHUB account > new_repository: name:shapes, description..., public/private, don t initialize!
# Deploy ------------------------------------------------------------------
